<script setup lang="ts" name="HomePost">
import type { TkPaginationProps } from "@teek/components/common/pagination";
import type { Post, TkContentData } from "@teek/config";
import { reactive, ref, watch, nextTick, computed, onMounted, watchEffect, useTemplateRef } from "vue";
...
import { useTeekConfig, usePosts, useWindowTransitionConfig } from "@teek/components/theme/config-provider";
import { TkPagination } from "@teek/components/common/pagination";
import { TkIcon } from "@teek/components/common/icon";
import { pageNumKey } from "./home-post";
import HomePostItemList from "./home-post-item-list.vue";
import HomePostItemCard from "./home-post-item-card.vue";

defineOptions({ name: "HomePost" });
...
const handlePagination = () => {
  ...
  nextTick(() => {
    ...
    const navHeight = removeUnit(rootStyles.getPropertyValue("--vp-c-text-1")) ?? 0;
    // 滚动返回时，减去导航栏的高度
...
const postItemInstance = useTemplateRef<HTMLLIElement[] | null>("postItemInstance");
const { start } = useWindowTransition(postItemInstance as any, false);
      ...
      <slot name="teek-home-post" v-bind="{ currentPosts, transitionName: postConfig.transitionName }">
        ...
        >
          <li v-for="post in currentPosts" :key="post.url" :class="{ 'full-img': coverImgMode === 'full' }">
            <div v-if="windowTransition" ref="postItemInstance" style="width: 100%; height: 100%">
              <HomePostItemCard v-if="postConfig.postStyle === 'card'" :post />
              <HomePostItemList v-else :post :coverImgMode />
            </div>
